...
tools_config = {
    ...
    "validate_data": {
        "function": tool_impls.validate_data,
...
VERTEX_TOOLS = [
                    ...
                    "intermediate_tables": {
                        ...
                        "items": {
                            ...
                            "properties": {
                                "dataset": {
                                    "type": "string",
                                    "description": "The dataset of the intermediate table",
                                },
                                "table": {
                                    "type": "string",
                                    "description": "The name of the intermediate table",
                                },
                                "description": {
                                    "type": "string",
                                    "description": "Description of the intermediate table",
                                },
                            },
                        },
                        "description": "The intermediate tables",
                    },
                    "data_quality_checks": {
                        "type": "object",
                        "description": "Data quality checks to be applied",
                    },
                },
    ...
    },
    {
        ...
        "function": {
            "name": "validate_data",
            "description": "Validates the data in the given table based on the provided validation rules.",
            "parameters": {
                "type": "object",
                "properties": {
                    "table": {"type": "string", "description": "The table to validate"},
                    "validation_rules": {
                        "type": "object",
                        "description": "The validation rules",
                    },
                },
                "required": ["table", "validation_rules"],
            },
        },
    },
]
...
def handle_request(state: AgentState) -> Dict:
    ...
    for source in source_tables:
        ...
        columns = tool_impls.query_information_schema(
    ...
    if target_tables:
        ...
        if all_same_dataset:
            ...
            if (
                ...
                and any(table["type"] == "fact" for table in target_tables)
            ):
                confirmation_message += (
    ...
    if state.target_tables:
        for target_table in state.target_tables:
            ...
            if state.data_quality_checks:
                results = tool_impls.validate_data(table_id, state.data_quality_checks)
...
workflow.add_edge("confirm_tables", "generate_code")
